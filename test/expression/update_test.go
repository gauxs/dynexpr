...
func TestAttributeUpdate(t *testing.T) {
	...
	rootExpBldr.BankDetails.AR().Accounts.Index(3).AR().BankAccountNumber.AddValue(dynexprv1.UPDATE_ADD, 2)
	rootExpBldr.BankDetails.AR().Accounts.Index(5).AddValue(dynexprv1.UPDATE_REMOVE, nil)

	rootExpBldr.PhoneNos.Index(14).AddValue(dynexprv1.UPDATE_SET, []*string{utils.PointerTo("7638927366")})

	// adding one more children
	utcLocation, _ := time.LoadLocation("UTC")
	rootExpBldr.FamilyDetails.AR().Children.AddValue(
		dynexprv1.UPDATE_SET,
		rootExpBldr.FamilyDetails.AR().Children.GetNameBuilder().ListAppend(
			expression.Value(test_models.Child{
		...
		var expectedValuesMap = map[string]*dynamodb.AttributeValue{
			...
			":2": {
				M: map[string]*dynamodb.AttributeValue{
					"account_type": {
						N: aws.String(strconv.Itoa(int(test_models.SAVINGS))),
					},
					"bank_account_number": {
